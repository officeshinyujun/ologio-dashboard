...
function filterEvents(events: Event[], tab: TabFilter): Event[] {
  ...
  if (tab === "academic")
    return events.filter((e) => e.scope.type === "school");
  if (tab === "class")
    return events.filter(
      (e) => e.scope.type === "department" || e.scope.type === "class",
    );
  if (tab === "special")
    return events.filter(
      (e) => e.event_type === "vacation" || e.event_type === "other",
    );
  return events;
}
...
function getScopeLabel(scope: EventScope): string {
  ...
  if (scope.type === "class")
    return `${scope.department} ${scope.grade}학년 ${scope.class}반`;
  return "개인";
}

function getTypeLabel(scope: EventScope): string {
  if (scope.type === "school") return "학사 일정";
  if (scope.type === "department" || scope.type === "class")
    return "학과/학년/반";
  return "특수 일정";
}
...
function formatDateOnly(dateVal: unknown): string {
  ...
  if (typeof dateVal === "object" && dateVal !== null) {
    if ("$date" in dateVal) {
      ...
      if (
        typeof dateInner === "object" &&
        dateInner !== null &&
        "$numberLong" in dateInner
      ) {
        const ms = parseInt(
          (dateInner as { $numberLong: string }).$numberLong,
          10,
        );
        if (!isNaN(ms)) {
        ...
        typeof dateInner === "string" ||
        typeof dateInner === "number"
      ) {
...
export default function SchedulePage() {
  ...
  const [formScopeType, setFormScopeType] = useState<
    "school" | "department" | "class"
  >("school");
  const [formDepartment, setFormDepartment] =
    useState<Department>("software_sw");
  ...
  const fetchEvents = useCallback(async () => {
    ...
    const res = await apiGet<Event[]>(
      `/admin/events?start=${monday.toISOString()}&end=${friday.toISOString()}`,
    );
  ...
  const handleSave = async () => {
    ...
    if (res.success) {
      ...
      alert(
        res.error?.message ||
          `저장에 실패했습니다. (${res.error?.code || "UNKNOWN"})`,
      );
    }
  };
  ...
  const filtered = events.filter((event) => {
    ...
    if (event.scope.type === "department") {
      if (selectedDept !== "all" && event.scope.department !== selectedDept)
        return false;
    } else if (event.scope.type === "class") {
      if (selectedDept !== "all" && event.scope.department !== selectedDept)
        return false;
      if (selectedGrade !== "all" && event.scope.grade !== selectedGrade)
        return false;
      if (selectedClass !== "all" && event.scope.class !== selectedClass)
        return false;
    ...
    >
      <HStack justify="between" align="center" fullWidth>
        <VStack gap={SPACING.s8}>
          ...
          <Typo.SM color="secondary">
            학사 일정, 학과/반별 일정, 특수 일정을 관리합니다.
          </Typo.SM>
        </VStack>
          ...
          >
            <div
              style={{
                ...
                height: 8,
                borderRadius: "50%",
                backgroundColor: COLORS.calendar[cat.color],
              }}
            />
        ...
        {TAB_FILTERS.map((tab) => (
          ...
          >
            <Typo.SM
              ...
              fontWeight={activeTab === tab.key ? "medium" : "regular"}
            >
              {tab.label}
            </Typo.SM>
          </button>
        ...
        {!loading && !error && (
          <>
            ...
            >
              ...
              <Typo.XS color="secondary" style={{ width: 80 }}>
                관리
              </Typo.XS>
            </HStack>
                  ...
                  <HStack align="center" gap={SPACING.s10} style={{ flex: 3 }}>
                    <div
                      style={{
                        ...
                        height: 20,
                        borderRadius: 2,
                        backgroundColor: COLORS.calendar[color],
                        flexShrink: 0,
                      }}
                    />
            ...
            {totalPages > 1 && (
              <HStack
                ...
                align="center"
                gap={SPACING.s8}
                style={{ marginTop: SPACING.s20, paddingBottom: SPACING.s20 }}
              >
                <button
                  onClick={() =>
          ...
          >
            ...
            <VStack gap={SPACING.s12} fullWidth>
              ...
              <VStack gap={SPACING.s6} fullWidth>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormScopeType(
                      e.target.value as "school" | "department" | "class",
                    )
                  }
                  style={{
                    width: "100%",
              ...
              {(formScopeType === "department" ||
                ...
                <VStack gap={SPACING.s6} fullWidth>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormDepartment(e.target.value as Department)
                    }
                    style={{
                      width: "100%",
                      padding: `${SPACING.s10}px ${SPACING.s12}px`,
            ...
            <HStack gap={SPACING.s10} justify="end" fullWidth>
              ...
              <button
                ...
                disabled={formSaving}
                style={{
                  padding: `${SPACING.s8}px ${SPACING.s20}px`,
                  backgroundColor: COLORS.brand.primary,
                  border: "none",
                  borderRadius: 8,
